...
APP_VERSION = "1.1.7 NO-COOK"
BLUEPRINT_VERSION = "1.0"
TARGET_PYTHON = "3.14"
BUILD_DATE = "2026-09-16 NO-COOK FIX"
...
def runtime_check() -> dict[str, str]:
    return {
        ...
        "PyMuPDF": getattr(fitz, "VersionBind", "not installed (fallback)") if fitz else "not installed (PyPDF2 fallback)",
    }
...
def extract_pdf_text(pdf_bytes: bytes, password: str = "") -> tuple[str, int, bool]:
    ...
    Fixes NoneType error + 0 chars + lower/upper PAN
    """
    if fitz is None:
        # Fallback to PyPDF2 if fitz not available - fixes NoneType error
        from PyPDF2 import PdfReader
        import io
        reader = PdfReader(io.BytesIO(pdf_bytes))
        encrypted = bool(reader.is_encrypted)
        if encrypted and password:
            for pw in [password, password.lower(), password.upper(), password.strip()]:
                try:
    ...
    if encrypted:
        ...
        # Try lower/upper PAN variants - fixes 0 chars with uppercase PAN
        ...
        for pw in uniq:
            try:
                ...
                continue
        # Original single try replaced by loop above
        _unused_auth = doc.authenticate(password) if False else auth_result
        if not auth_result:
            doc.close()
            raise ValueError(
...
def fetch_upstream_csv(relative_path: str) -> pd.DataFrame:
    # FIXED: No network at startup - prevents infinite cooking on Streamlit Cloud
    # GitHub raw.githubusercontent.com is often blocked/slow, causing hang
    # Return empty - user can upload CSV manually if needed
    return pd.DataFrame()


@st.cache_data(ttl=3600, show_spinner=False)
def fetch_symbol_renames() -> pd.DataFrame:
    # FIXED: No network - prevents infinite cooking
    return pd.DataFrame()
