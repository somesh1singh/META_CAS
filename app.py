...
def extract_pdf_text(pdf_bytes: bytes, password: str = "") -> tuple[str, int, bool]:
    ...
    Fixes: tries lower/upper PAN + fallback to PyPDF2 if fitz missing (fixes NoneType error)
    Passwords are used transiently only for PDF authentication and are not persisted.
    """
    # Try fitz/pymupdf first if available
    if fitz is not None:
        try:
            doc = fitz.open(stream=pdf_bytes, filetype="pdf")
            encrypted = bool(doc.needs_pass or getattr(doc, 'is_encrypted', False))
            if encrypted:
                if not password:
                    doc.close()
                    raise ValueError("This PDF is password protected. Enter the PDF password and try again.")
                # Try all case variants - fixes 0 chars issue with uppercase PAN
                ...
                if not authed:
                    doc.close()
                    raise ValueError(f"The PDF password is incorrect. Tried {uniq}. Please check lowercase PAN e.g. ckcps3557g")
            pages=[]
            try:
                for page in doc:
                    pages.append(page.get_text("text") or page.get_text() or "")
                page_count=len(doc)
            finally:
                doc.close()
            text="\n".join(pages)
            if len(text.strip())>50:
                return text, page_count, encrypted
        except ValueError:
            raise
    ...
    # Fallback to PyPDF2 - fixes ModuleNotFoundError: fitz and NoneType error
    try:
        ...
        if encrypted:
            ...
            pw_tries = [password, password.lower(), password.upper(), password.strip()]
            decrypted=False
            for pw in pw_tries:
                try:
                    ...
                    continue
            if not decrypted:
                raise ValueError(f"Password incorrect. Tried lower/upper PAN. Use lowercase PAN like ckcps3557g")
        pages=[p.extract_text() or "" for p in reader.pages]
        return "\n".join(pages), len(reader.pages), encrypted
    except ValueError:
        raise
    except Exception as e:
        pass
    
    # Last fallback to pdfplumber
    try:
        import pdfplumber
        import io
        pw_tries = [password, password.lower(), password.upper()] if password else [""]
        for pw in pw_tries:
            try:
                with pdfplumber.open(io.BytesIO(pdf_bytes), password=pw) as pdf:
                    ...
                    if len(text.strip())>10:
                        return text, len(pdf.pages), bool(password)
            except:
                continue
    except Exception:
        pass
    
    raise ValueError("Failed to extract text - PDF may be scanned or password wrong. Try lowercase PAN ckcps3557g")
